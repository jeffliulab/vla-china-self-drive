...
"""
欢迎使用 CARLA 无渲染模式可视化器（No-Rendering Mode Visualizer）

    TAB          : 切换 hero 模式
    Mouse Wheel  : 放大 / 缩小
    Mouse Drag   : 移动地图（仅地图模式下可用）
...
# 我们将使用 Tango Desktop Project 所采用的调色板（每种颜色按亮度等级编号）
# 参见：https://en.wikipedia.org/wiki/Tango_Desktop_Project
...
class Util(object):
    ...
    def blits(destination_surface, source_surfaces, rect=None, blend_mode=0):
        """将所有源 surface 渲染到一个目标 surface 上的函数"""
    ...
    def get_bounding_box(actor):
        """获取 actor 在世界坐标系下的 bounding box 各角点"""
...
class HUD (object):
    """负责渲染 HUD 的类，HUD 显示关于 world 和 hero 车辆的信息"""

    def __init__(self, name, width, height):
        """初始化默认的 HUD 参数以及将要显示的内容数据参数"""
        self.name = name
    ...
    def render_vehicles_ids(self, vehicle_id_surface, list_actors, world_to_pixel, hero_actor, hero_transform):
        """当该标志启用时，显示 world 中已 spawn 车辆的 ID。根据车辆类型的不同，
        会以不同的颜色渲染"""
...
class TrafficLightSurfaces(object):
    """保存用于绘制交通信号灯的 surface（已缩放和旋转）"""

    def __init__(self):
        def make_surface(tl):
            """绘制一个交通信号灯，它由一个深色背景 surface 和 3 个圆圈组成，圆圈颜色根据信号灯状态而定"""
            w = 40
...
class MapImage(object):
    """负责从俯视视角渲染 carla world 的 2D 图像的类。请注意这里使用了缓存系统，因此如果某个 Carla 城镇的 OpenDrive 内容
    没有变化，它会读取并使用之前某次执行中渲染好的已存储图像"""

    def __init__(self, carla_world, carla_map, pixels_per_meter, show_triggers, show_connections, show_spawn_points):
        """ 根据 world、其地图以及提供路网额外信息的附加标志，渲染生成的地图图像"""
        ...
        # Pygame surface 的最大尺寸
        width_in_pixels = (1 << 14) - 1

        # 调整每米对应的像素数，使整个 world 能放进 surface 中
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        """绘制所有道路，包括车道标线、箭头和交通标志"""
        map_surface.fill(COLOR_ALUMINIUM_4)
        precision = 0.05

        def lane_marking_color_to_tango(lane_marking_color):
            """将 PythonAPI 中定义的车道标线颜色枚举映射为 Tango 颜色"""
        ...
        def get_lane_markings(lane_marking_type, lane_marking_color, waypoints, sign):
            """对于多种车道标线类型（SolidSolid、BrokenSolid、SolidBroken 和 BrokenBroken），将它们
             转换为虚线（Broken）和实线（Solid）的组合"""
            margin = 0.25
        ...
        def draw_lane_marking_single_side(surface, waypoints, sign):
            """根据给定的一组 waypoint 绘制车道标线，并依据 sign 参数决定绘制 waypoint 的右侧还是
            左侧"""
            lane_marking = None
        ...
        def draw_traffic_signs(surface, font_surface, actor, color=COLOR_ALUMINIUM_2, trigger_color=COLOR_PLUM_0):
            """绘制 stop 交通标志，并在启用时绘制其 bounding box"""
            ...
            # 绘制 stop trigger 的 bounding box
        ...
        def lateral_shift(transform, shift):
            """对一个 transform 的前向向量进行横向偏移"""
            transform.rotation.yaw += 90
            return transform.location + shift * transform.get_forward_vector()

        def draw_topology(carla_topology, index):
            """ 通过生成 waypoint 来绘制交通标志，以及包含人行道、停车区和路肩的路网"""
            ...
            for waypoint in topology:
                ...
                # 生成某个 road id 的 waypoint。当 road id 发生变化时停止
                ...
                for w in waypoints:
                    # 向左侧遍历，对车道类型进行分类，直到没有更多 waypoint
                    l = w.get_left_lane()
                    ...
                    # 向右侧遍历，对车道类型进行分类，直到没有更多 waypoint
                    r = w.get_right_lane()
...
class World(object):
    ...
    def start(self, hud, input_control):
        """构建地图图像，保存所需的模块，并为 Hero 模式下的渲染做准备"""
    ...
    def select_hero_actor(self):
        """如果存在多个 hero actor，则只选择其中一个。如果一个都没有，则 spawn 一个。"""
    ...
    def tick(self, clock):
        """为 Hero 模式和 Map 模式获取 actor，并据此更新 HUD"""
        actors = self.world.get_actors()

        # 我们同时保存这些 transform，以避免在渲染时混用
        # 上一 tick 和当前 tick 的 transform。
    ...
    def update_hud_info(self, clock):
        """更新 HUD 信息，包括仿真状态、hero 模式以及是否有交通信号灯正在影响 hero actor"""
    ...
    def _render_traffic_lights(self, surface, list_tl, world_to_pixel):
        """渲染交通信号灯，并在相应标志启用时显示其 trigger 和 bounding box"""
        self.affected_traffic_light = None
    ...
    def _render_speed_limits(self, surface, list_sl, world_to_pixel, world_to_pixel_width):
        """通过绘制两个同心圆（外圈为红色，内圈为白色）和一段限速文本来渲染限速标志"""
        ...
        for sl in list_sl:
            ...
            # Blit
            if self.hero_actor is not None:
                # 在 hero 模式下，相对于 hero 车辆前方旋转字体 surface
    ...
    def _render_walkers(self, surface, list_w, world_to_pixel):
        """渲染行人（walker）的 bounding box"""
        for w in list_w:
            color = COLOR_PLUM_0

            # 计算 bounding box 的各个点
    ...
    def render(self, display):
        """在 hero 模式和地图模式下渲染地图及所有 actor"""
        if self.actors_with_transforms is None:
            return
        self.result_surface.fill(COLOR_BLACK)

        # 按车辆 type id 对 actor 进行分类
