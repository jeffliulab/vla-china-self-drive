...
"""
在同一个 pygame 窗口中渲染多个 sensor 的脚本

默认渲染四个 camera、一个 LiDAR 和一个 Semantic LiDAR。
可以很方便地配置成任意数量的 sensor。
具体做法见第 290-308 行。
"""
...
def run_simulation(args, client):
    """该函数使用传入的 args 参数、并连接传入的 carla client
    执行一次测试运行。
    """
    ...
    try:
        ...
        # 实例化将要挂载 sensor 的 vehicle
        bp = world.get_blueprint_library().filter('charger_2020')[0]
        vehicle = world.spawn_actor(bp, random.choice(world.get_map().get_spawn_points()))
        vehicle_list.append(vehicle)
        vehicle.set_autopilot(True)

        # Display Manager 负责在一个窗口中组织所有 sensor 及其显示
        # 可以很方便地配置网格布局和窗口总尺寸
        display_manager = DisplayManager(grid_size=[2, 3], window_size=[args.width, args.height])

        # 然后，可以用 SensorManager 按需 spawn RGBCamera、LiDAR 和 SemanticLiDAR，
        # 并把每一个分配到一个网格位置，
